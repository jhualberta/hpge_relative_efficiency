...
G4VPhysicalVolume* GeDetectorConstruction::Construct()
{
  ...
  // World
  //------------------------------ 
  solidWorld= new G4Box("World",950,950,950);
 ...
 //--------------------------------
  ...
  CoSrcMaterial = man->FindOrBuildMaterial("G4_PLASTIC_SC_VINYLTOLUENE");
 
  solidCoSrc = new G4Tubs("CoSrc", 0, 4, 2, 0, 2*M_PI );
  logicCoSrc = new G4LogicalVolume(solidCoSrc,CoSrcMaterial,"CoSrcLV",0,0,0);
  physiCoSrc = new G4PVPlacement(0,
  				     G4ThreeVector(0,0,7.5*25.4/2.0+3.5),
				     "CoSrc",
				     logicCoSrc,
				     physiDetBox,
				     false,
    ...
    //----------------------------
    // Sample parts      //Commented - tehre is a problem they are placed in the detector - wrongly rotated
    //----------------------------
    // Bowtech lights approximation
 /*   G4RotationMatrix *rotDs = new G4RotationMatrix;
    rotDs->rotateX(M_PI/3);
    sampleBowtechLight_p1 = new G4Tubs("SampleLightP1",70.45*mm,76*mm,20*mm,0,2*M_PI);
    logicSampleBowtechLight_p1 = new G4LogicalVolume(sampleBowtechLight_p1,SSMaterial,"sampleLightP1LV",0,0,0);
    physiSampleBowtechLigh_p1 = new G4PVPlacement(rotDs,
						 G4ThreeVector(0,0,-20*mm),
						 "SampleLightP1PV",
						 logicSampleBowtechLight_p1, 
						 physiDetBox,
						 false,
						 0);

    sampleBowtechLight_p2 = new G4Tubs("SampleLightP2",27.15*mm,31.75*mm,20*mm,0,2*M_PI);
    logicSampleBowtechLight_p2 = new G4LogicalVolume(sampleBowtechLight_p2,SSMaterial,"sampleLightP2LV",0,0,0);
    physiSampleBowtechLigh_p2 = new G4PVPlacement(rotDs,
						 G4ThreeVector(0,0,20*mm),
						 "SampleLightP2PV",
						 logicSampleBowtechLight_p2, 
						 physiDetBox,
						 false,
						 0);
...
*/
 ...
 G4VisAttributes * grayAndSolid  = new G4VisAttributes(G4Colour(0.1,0.5,0.8,0.75));
  ...
  logicGeDetF          ->SetVisAttributes(yellowAndSolid);
  ...
  // source
  logicCoSrc           ->SetVisAttributes(redAndSolid);
